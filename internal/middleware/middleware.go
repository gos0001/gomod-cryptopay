...
func (m *Middleware) APIKey() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Every configured key is compared, and the loop is not short-circuited
		// on a match, so the time taken does not depend on which key matched or
		// on how far into the header the first differing byte is.
		var ok int
